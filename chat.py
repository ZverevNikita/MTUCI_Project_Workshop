...
import models
import time
from authentication_funcs import *

async def main():
    clear()
    choices = ['Регистрация'] + ['Вход']
    choice = await actions('Выберите действие', buttons=choices)
    if choice == 'Регистрация':
        while True:
            user_data = await input_group("Заполните форму", [
            input('Логин', name='login', required=True,
              help_text='Минимум 3 символа',validate=login_limit,maxlength=20),
            input('Отображаемое имя',help_text='Оставьте пустым, чтобы использовать логин для имени', name='username', required=False),
            input('Пароль', name='password',help_text='Минимум 6 символов', type=PASSWORD, required=True,validate=password_limit, maxlength= 72),
            input('Подтвердите пароль', name='confirm', type=PASSWORD, required=True),
            ])
            if user_data['password'] != user_data['confirm']:
                toast("Пароли не совпадают", color='error')
            else:
                clear()
                user_data['password'] = bcrypt.hashpw(user_data['password'].encode(), bcrypt.gensalt(15)).decode()
                if login_exist(user_data['login']) == True:
                    toast("Логин уже занят", color='error')
                else:
                    ...
                    database.execute("INSERT INTO Users VALUES (?, ?, ?, ?);",
                                     (user_data['login'], user_data['username'], user_data['password'], registration_date))
                    database.commit()
                    database.close()
                    nickname = get_username(user_data['login'])
                    toast(f"Пользователь {nickname} успешно зарегистрирован!", color='green')
                    break
    if choice == 'Вход':
        while True:
            put_text("🔐 Вход в систему")
            user_data = await input_group("Вход", [
                input('Логин', name='login', required=True,
                      help_text='Минимум 3 символа', validate=login_limit, maxlength=20),
                input('Пароль', name='password', help_text='Минимум 6 символов', type=PASSWORD, required=True,
                      validate=password_limit, maxlength=72),
            ])
            if login_exist(user_data['login']):
                if password_check(user_data['login'], user_data['password']):
                    nickname = get_username(user_data['login'])
                    toast(f"Рады видеть вас снова, {nickname}!", color='green')
                    break
                else:
                    toast("Неверный логин или пароль", color='error')
            else:
                bcrypt.checkpw(user_data['password'].encode(),
                               '$2a$15$gcaFhC1m0sWi6YkXg5vF/OrdBVrfRqS8yQpSJBopcxgtsnWxoilga'.encode())
                toast("Неверный логин или пароль", color='error')
    ...
    while True:
        ...
        async with models.data_lock:
            ...
            if len(models.rooms[room_name]['messages']) > models.MAX_MESSAGES_PER_ROOM:
                models.rooms[room_name]['messages'] = models.rooms[room_name]['messages'][
                    -models.MAX_MESSAGES_PER_ROOM:]
                ...
                async with models.data_lock:
                    ...
                    if current_room:
                        ...
                        if len(models.rooms[current_room]['messages']) > models.MAX_MESSAGES_PER_ROOM:
                            models.rooms[current_room]['messages'] = models.rooms[current_room]['messages'][
                                -models.MAX_MESSAGES_PER_ROOM:]
                        with use_scope('msg-box'):
